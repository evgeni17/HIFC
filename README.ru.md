...
## Геопривязка и верхние уровни

Импорт кладёт в detail верхние уровни файла: `s@ifc_crs`, `d@ifc_georef` (переход в систему координат карты,
сама система, истинный север), `d@ifc_project`, `d[]@ifc_sites` (широта/долгота, отметка, размещение)
и `d[]@ifc_facilities` (здания и сооружения с размещением). У каждой площадки и здания есть `xform`,
готовая для `hou.Matrix4(...)`. Сдвиг карты к геометрии не применяется; все поля — в справке ноды (F1).
Экспорт геопривязки пока не пишет.

`4@global_xform` — матрица самого верхнего уровня размещения (корневой площадки). Transform By Attribute
с Attribute = `global_xform` и *Invert Transformation* ставит модель к началу координат. Для моделей, которые лежат
далеко, включите на ноде импорта **Move to Origin**: тот же перенос делается в двойной точности до записи позиций
во float32, и ничего не теряется; Transform By Attribute без Invert возвращает модель на место.
